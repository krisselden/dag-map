(function(QUnit, DAG) {
  ...
  QUnit.test("detects circular dependencies when added", function(assert) {
    ...
    assert.throws(
      ...
      new Error(
        "cycle detected: imposible <- eat omelette <- fry omelette <- shake eggs <- imposible"
      ),
  ...
  QUnit.test(
    "#topsort iterates over the edges respecting precedence order",
    function(assert) {
      var graph = new DAG();
      var names = [];
      var index = 0;
      ...
      assert.ok(
        names.indexOf("buy eggs") < names.indexOf("shake eggs"),
        "you need eggs to shake them"
      );
      assert.ok(
        names.indexOf("buy oil") < names.indexOf("warm oil"),
        "you need oil to warm it"
      );
      assert.ok(
        names.indexOf("eat omelette") < names.indexOf("clear the table"),
        "you clear the table after eat"
      );
      assert.ok(
        names.indexOf("fry omelette") < names.indexOf("eat omelette"),
        "cook before eat"
      );
      assert.ok(
        names.indexOf("shake eggs") < names.indexOf("fry omelette"),
        "shake before put into the pan"
      );
      assert.ok(
        names.indexOf("prepare salad") > -1,
        "we don't know when we prepare the salad, but we do"
      );
    }
  );

  QUnit.test(
    "#add supports both strings and arrays to specify precedences",
    function(assert) {
      var graph = new DAG();
      var names = [];
      var index = 0;
      ...
      graph.add(
        "buy oil",
        5,
        ["fry omelette", "shake eggs", "prepare the table"],
        ["warm oil"]
      );
